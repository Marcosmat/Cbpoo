...
public class Autor extends Especialista implements AutorInterface {
	...
	//********************************************
	public void verAvaliacoesArtigo() {
		
		System.out.printf("\nDigite codigo do artigo desejado: ");
		try {
			
			int cod = l.nextInt();
			l.nextLine();
				
				if(Cbpoo.getInst().getArtigosAceitos()!=null) {
					
					for(Artigo i: Cbpoo.getInst().getArtigosAceitos()) 
						this.visualizaAvAutor(i, cod, this.getCpf());
					...
					System.out.println("Nao ha artigos submetidos!");
					return;
				}
